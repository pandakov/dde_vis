...
with st.sidebar:
    ...
    st.caption(f"Используется: **{ 'загруженный файл' if uploaded else default_path }**")
...
with st.spinner("Загрузка данных..."):
    if uploaded:
        ...
        if not Path(default_path).exists():
            st.error(f"Файл {default_path} не найден. Загрузите .parquet через сайдбар.")
            st.stop()
...
with st.sidebar:
    st.header("🎛️ Фильтры")
    sex_options = sorted(df["sex"].dropna().unique().tolist()) if "sex" in df.columns else []
    class_options = sorted(df["pclass"].dropna().unique().tolist()) if "pclass" in df.columns else []
    survived_options = sorted(df["survived"].dropna().unique().tolist()) if "survived" in df.columns else []

    # Диапазоны
    age_min, age_max = (int(df["age"].min()), int(df["age"].max())) if "age" in df.columns else (0, 100)
    fare_min, fare_max = (float(df["fare"].min()), float(df["fare"].max())) if "fare" in df.columns else (0.0, 600.0)

    # Восстановление из query params
    sex_sel = st.multiselect("Пол", options=sex_options, default=get_param_list("sex", sex_options[:]))
    pclass_sel = st.multiselect("Класс (pclass)", options=class_options, default=get_param_list("pclass", class_options[:]))
    survived_sel = st.multiselect("Выжил (0/1)", options=survived_options, default=get_param_list("survived", survived_options[:]))

    age_range = st.slider("Возраст", min_value=age_min, max_value=age_max,
                          value=(age_min, age_max), step=1, disabled="age" not in df.columns)
    fare_range = st.slider("Тариф (fare)", min_value=float(fare_min), max_value=float(fare_max),
                           value=(float(fare_min), float(fare_max)), step=1.0, disabled="fare" not in df.columns)
...
def between_num(s, lo, hi):
    return s.between(lo, hi) if pd.api.types.is_numeric_dtype(s) else pd.Series([True]*len(s), index=s.index)
...
if "sex" in filtered.columns:
    col_cfg["sex"] = st.column_config.SelectboxColumn("sex", options=sorted(filtered["sex"].dropna().unique().tolist()))
if "pclass" in filtered.columns:
    col_cfg["pclass"] = st.column_config.NumberColumn("pclass", help="Passenger class (1/2/3)")
...
# ---------- Tabs for visuals ----------
tab2d, tab3d, tabdist, tabpivot = st.tabs(["✨ 2D Scatter", "📐 3D Scatter", "📊 Распределения", "🧮 Сводные/Теплокарта"])

with tab2d:
    x_axis = st.selectbox("X-axis", filtered.columns, index=(filtered.columns.get_loc("age") if "age" in filtered.columns else 0))
    y_axis = st.selectbox("Y-axis", filtered.columns, index=(filtered.columns.get_loc("fare") if "fare" in filtered.columns else 1))
    color = st.selectbox("Color by", filtered.columns, index=(filtered.columns.get_loc("sex") if "sex" in filtered.columns else 0))
    size = st.selectbox("Size by (опционально)", ["—"] + filtered.columns.tolist(), index=0)
    ...
    st.download_button("⬇️ Скачать график (HTML)", data=html_bytes, file_name="scatter_2d.html", mime="text/html")

with tab3d:
    st.caption("Совет: для 3D лучше выбирать числовые оси.")
    x3 = st.selectbox("X", [c for c in filtered.columns if pd.api.types.is_numeric_dtype(filtered[c])], index=0)
    y3 = st.selectbox("Y", [c for c in filtered.columns if pd.api.types.is_numeric_dtype(filtered[c])], index=1 if len([c for c in filtered.columns if pd.api.types.is_numeric_dtype(filtered[c])]) > 1 else 0)
    z3 = st.selectbox("Z", [c for c in filtered.columns if pd.api.types.is_numeric_dtype(filtered[c])], index=2 if len([c for c in filtered.columns if pd.api.types.is_numeric_dtype(filtered[c])]) > 2 else 0)
    color3 = st.selectbox("Color by", filtered.columns, index=(filtered.columns.get_loc("sex") if "sex" in filtered.columns else 0))
    fig3d = px.scatter_3d(filtered, x=x3, y=y3, z=z3, color=color3, title="Titanic 3D Visualization (filtered)")
...
with tabdist:
    ...
    with c1:
        if "sex" in filtered.columns and "survived" in filtered.columns:
            fig_hist = px.histogram(filtered, x="sex", color="survived", barmode="group", title="Survival by Sex")
    ...
    with c2:
        if "age" in filtered.columns:
            fig_age = px.histogram(filtered, x="age", nbins=30, marginal="box", title="Age Distribution")
            st.plotly_chart(fig_age, use_container_width=True)
        if "fare" in filtered.columns:
            fig_fare = px.histogram(filtered, x="fare", nbins=40, marginal="violin", title="Fare Distribution")
            st.plotly_chart(fig_fare, use_container_width=True)

with tabpivot:
    st.caption("Сводная по средней выживаемости (можно менять оси).")
    rows = st.selectbox("Строки", options=[c for c in filtered.columns if filtered[c].dtype.name not in ("float64", "float32")], index=0 if "sex" in filtered.columns else 0)
    cols = st.selectbox("Столбцы", options=[c for c in filtered.columns if filtered[c].dtype.name not in ("float64", "float32")], index=0 if "pclass" in filtered.columns else 0)
    if "survived" in filtered.columns:
        pv = filtered.pivot_table(index=rows, columns=cols, values="survived", aggfunc="mean")
        pv = pv.sort_index().sort_index(axis=1)
        st.dataframe((pv * 100).round(1).astype(str) + " %", use_container_width=True)
        fig_heat = px.imshow(pv, text_auto=".1%", aspect="auto", title="Heatmap: Survival Rate")
...
st.download_button("⬇️ Скачать отфильтрованные данные (CSV)", data=csv_bytes, file_name="titanic_filtered.csv", mime="text/csv")
